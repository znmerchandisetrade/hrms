...
// Get change dayoff details
$query = "SELECT cdo.*, u.full_name, u.company_id, u.department_id, 
          ...
          LEFT JOIN users approver ON cdo.approved_by = approver.id
          LEFT JOIN roles approver_role ON approver.role_id = approver_role.id
          WHERE cdo.id = :id AND cdo.status = 'approved'";
...
// Format dates
$effective_date = date('F j, Y', strtotime($application['effective_date']));
...
<head>
    ...
    <title>Change of Dayoff - <?php echo htmlspecialchars($application['full_name']); ?></title>
    <style>
        @media print {
            @page { margin: 0.5in; }
            body { font-size: 11pt; }
...
<body>
    <div class="header">
        ...
        <div class="document-title">CHANGE OF DAYOFF APPROVAL</div>
    </div>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-row">
                <div class="info-label">Current Dayoff:</div>
                <div class="info-value"><strong><?php echo htmlspecialchars($application['current_dayoff']); ?></strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Requested Dayoff:</div>
                <div class="info-value"><strong><?php echo htmlspecialchars($application['requested_dayoff']); ?></strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Effective Date:</div>
                <div class="info-value"><strong><?php echo $effective_date; ?></strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Reason for Change:</div>
                <div class="info-value"><?php echo nl2br(htmlspecialchars($application['reason'])); ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="footer">
        Document ID: CDO-<?php echo str_pad($application_id, 6, '0', STR_PAD_LEFT); ?> | 
