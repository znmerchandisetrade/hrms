...
if ($_POST) {
    ...
    try {
        ...
        $check_query = "SELECT id FROM id_renewal_applications 
        ...
        $delete_query = "DELETE FROM id_renewal_applications WHERE id = :id";
        $delete_stmt = $db->prepare($delete_query);
        $delete_stmt->bindParam(":id", $application_id);

        if ($delete_stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'ID renewal application cancelled successfully.']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Failed to cancel application.']);
        }

    } catch (Exception $e) {
        error_log("Cancel ID Renewal Error: " . $e->getMessage());
