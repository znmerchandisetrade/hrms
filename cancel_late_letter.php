...
if ($_POST) {
    ...
    try {
        ...
        $delete_query = "DELETE FROM late_letter_applications WHERE id = :id";
        $delete_stmt = $db->prepare($delete_query);
        $delete_stmt->bindParam(":id", $application_id);

        if ($delete_stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Late letter application cancelled successfully.']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Failed to cancel application.']);
        }

    } catch (Exception $e) {
        error_log("Cancel Late Letter Error: " . $e->getMessage());
